...
class ZYKeyboardTextView: UITextView {

    func insertEmotion(emoticon: ZYKeyboardEmoticon) {
        ...
        if let tempPngPath = emoticon.pngPath {
            ...
            //2.2.创建图片的属性字符串
            let attachment = ZYKeyboardAttachment()
    ...
    func emoticonStr() -> String {
        ...
        attributedText.enumerateAttributesInRange(range, options: NSAttributedStringEnumerationOptions(rawValue: 0)) { (dict, range, _) in
            ...
            if let tempAttachment = dict["NSAttachment"] as? ZYKeyboardAttachment {
