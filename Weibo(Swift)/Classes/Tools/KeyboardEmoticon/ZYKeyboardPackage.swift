 ...
 */
class ZYKeyboardPackage: NSObject {
    ...
    ///当前组所有的表情
    var emoticons: [ZYKeyboardEmoticon]?
    ...
    ///加载所有组数据
    class func loadEmoticonPackages() -> [ZYKeyboardPackage] {
        
        var models = [ZYKeyboardPackage]()
        
        //0.手动添加最近组
        let package = ZYKeyboardPackage(id: "")
        package.appendEmptyEmoticons()
        models.append(package)
        
        //1.加载emoticons.plist文件
        //1.1.获取emoticons.plist文件路径
        let path = NSBundle.mainBundle().pathForResource("emoticons.plist", ofType: nil, inDirectory: "Emoticons.bundle")
        
        //1.2.加载emoticons.plist
        ...
        for packageDict in array {
            ...
            //2.1.创建当前组模型
            let package = ZYKeyboardPackage(id: packageDict["id"] as! String)
            
            //2.2.加载当前组所有的表情数据
            package.loadEmoticons()
            
            //2.3.补全一组数据, 保证当前组能被21整除
            package.appendEmptyEmoticons()
    ...
    private func loadEmoticons()
    {
        ...
        for emoticonDict in array
        {
            if index == 20
            {
                let emoticon = ZYKeyboardEmoticon(isRemoveButton: true)
                models.append(emoticon)
                index = 0
                continue
            }
            
            let emoticon = ZYKeyboardEmoticon(dict: emoticonDict, id: self.id!)
    ...
    private func appendEmptyEmoticons()
    {
        ...
        for _ in number..<20
        {
            let emoticon = ZYKeyboardEmoticon(isRemoveButton: false)
    ...
    ///添加最近表情
    func addFavoriteEmoticon(emoticon: ZYKeyboardEmoticon)
    {
        ...
        //4.添加一个删除按钮
        emoticons?.append(ZYKeyboardEmoticon(isRemoveButton: true))
    }
}

extension ZYKeyboardPackage
{
    ...
    {
        //1.加载所有的表情
        let packages = ZYKeyboardPackage.loadEmoticonPackages()
        
        //2.遍历所有的表情包,拿到chs对应的pngPath
        for package in packages {
            //2.1.如果表情包没有值,则表情包不需要再进行遍历
            ...
            //2.4.如果遍历了一个表情包中已经取出了pngPath,则不需要再遍历下一个表情包
            if pngPath != nil {
                return pngPath
            }
        }
        
        //3.如果遍历了所有的表情包依然没有值,则直接返回nil
        return nil
    }
}

class ZYKeyboardEmoticon: NSObject {
