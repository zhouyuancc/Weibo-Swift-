...
//  Weibo(Swift)
//
//  Created by ZhouYuan on 16/10/12.
...
class ComposeViewController: UIViewController {
    ...
    ///照片选中控制器的属性
    var photoPickerVc : ZYPhotoPickerCollectionViewController?
    
    //懒加载表情键盘的控制器
    lazy var keyboardEmoticonVc: ZYKeyboardEmoticonViewController = ZYKeyboardEmoticonViewController {[unowned self] (emoticon) in
        
//        ZYLog(emoticon)
        self.statusTextView.insertEmotion(emoticon)
    ...
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        if segue.identifier == "photoPicker" {
            photoPickerVc = segue.destinationViewController as? ZYPhotoPickerCollectionViewController
        }
    }
